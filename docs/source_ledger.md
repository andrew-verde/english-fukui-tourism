...
| Cross-language monthly trends | EN/JP/CN monthly volume + within-group sentiment, side-project only | hypothesis (awaiting local regeneration of row-level multilingual reviews) | `make cross-language-trends` | `scripts/build_cross_language_trends.py` | `output/multilingual_review_analysis/reviews_multilingual.csv` + `output/chinese_social_media_analysis/tagged_chinese_social_posts.csv` | `output/cross_language_trends/` — sentiment scales group-specific, never merged |
